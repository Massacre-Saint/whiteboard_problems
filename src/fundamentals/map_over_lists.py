...
The second should be a lambda function. The function should map the lambda function over the contents of the inner lists
'''
def map_it(input, func):
  mutated_nested_list = []
  for inner_lists in input:
    mutated_inner_list = [func(element) for element in inner_lists]
    mutated_nested_list.append(mutated_inner_list)
  return mutated_nested_list
